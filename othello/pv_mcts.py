...
PV_EVALUATE_COUNT = 1000 # 1推論あたりのシミュレーション回数 (本家は1000)
    ...
    class Node:
        ...
        def evaluate(self):
            ...
            if not self.child_nodes:
                ...
                value = -self.next_child_node().evaluate() # 相手から見た価値なので反転
                self.w += value
                self.n += 1
                return value
...
class Node:
    ...
    def evaluate(self, predict_policy, predict_value):
        ...
        if not self.child_nodes:
            ...
            # アーク評価値が最大の子ノードの評価で価値を取得
            value = -self.child_nodes[self.mem_next_child].evaluate(predict_policy, predict_value) # 相手から見た価値なので反転
            self.w += value
            self.n += 1
            return value
    ...
    def next_child_node(self):
        ...
        pucb_values = [(-child_node.w / child_node.n if child_node.n else 0.0) + C_PUCT * child_node.p * sqrt_t / (1 + child_node.n) for child_node in self.child_nodes]
        self.mem_next_child = np.argmax(pucb_values)
        return self.child_nodes[self.mem_next_child]

def call_node_evaluate(r):
    r[0].evaluate(r[1], r[2])
